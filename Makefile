...
macosx : OS := MACOSX
macosx : CFLAGS += -D_GLFW_USE_OPENGL -D_GLFW_COCOA -D_GLFW_BUILD_ALL -D_GLFW_NSGL
macosx :
	# del old so
	rm -fr *.so
	gcc -c -O3 gl_2_0_core.c
	gcc -c -O3 nanovg/src/nanovg.c
	ar rcs libnanovg.a *.o
	rm -fr *.o
	gcc  -O3 -bundle -undefined dynamic_lookup -o nvg.so nanovg.def lua-nanovg.c libnanovg.a
	...
	gcc -c -O3 $(CFLAGS) glfw/src/cocoa_window.m
	gcc -c -O3 $(CFLAGS) glfw/src/iokit_joystick.m
	gcc -c -O3 $(CFLAGS) glfw/src/mach_time.c
	gcc -c -O3 $(CFLAGS) glfw/src/posix_tls.c
	gcc -c -O3 $(CFLAGS) glfw/src/nsgl_context.m
	ar rcs libglfw3.a *.o
	rm -fr *.o
	gcc -O3 -bundle -undefined dynamic_lookup -o glfw.so lua-glfw.c libglfw3.a -framework OpenGL -framework CoreGraphics -framework Foundation -framework Cocoa -framework IoKit -framework CoreVideo
